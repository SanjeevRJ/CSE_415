...
This file contains an edited version of UCS.py, which now uses heuristics
to implement an A* search algorithm.
'''
...
g = {} # We will use a global hash table to associate f values with states.
global h
h = Problem.h
...
# STEP 1a. Put the start state on a priority queue called OPEN after assigning f = h
...
#         If S is a goal state, output its description
    ...
    if Problem.GOAL_TEST(S):
      ...
      if (h(initial_state) - h(S)) <= TOTAL_COST: 
         return path
    COUNT += 1
